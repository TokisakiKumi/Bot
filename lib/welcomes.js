...
module.exports = welcomes = async (client, event) => {
    //console.log(event.action)
    const welkoms = JSON.parse(fs.readFileSync('./lib/welcomes.json'))
    const isWelkoms = welkoms.includes(event.chat)
    try {
        if (event.action == 'add' && isWelkoms) {
